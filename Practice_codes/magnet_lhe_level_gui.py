"""GUI for the magnet liquid-helium level probe.

LabVIEW mapping from the screenshot:
  - VISA resource: ASRL5::INSTR / "Port6-AMI-135"
  - Loop delay: 500 ms
  - Query text: level\r
  - Displayed level: parsed numeric value * 0.393

Run from instrument_readers:
    python level/magnet_lhe_level_gui.py
"""
...
# Import the driver from the companion module (same directory).
from magnet_lhe_level import (  # noqa: F401  (re-exported for external callers)
    MagnetLevelRead,
    MagnetLHeLevelReader,
    fmt,
    is_finite,
)


class MagnetLHeLevelGui(tk.Tk):
    BG = "#0f1720"
    PANEL = "#172231"
    PANEL_2 = "#202d3f"
    BORDER = "#304156"
    ...
    def __init__(self) -> None:
        super().__init__()
        self.title("Magnet LHe Level - AMI 135")
        self.geometry("1020x700")
        self.minsize(900, 620)
        self.configure(bg=self.BG)

        self.port_var = tk.StringVar(value="ASRL5::INSTR")
        self.baud_var = tk.IntVar(value=9600)
        self.backend_var = tk.StringVar(value="auto")
        self.command_var = tk.StringVar(value=r"level\r")
        self.scale_var = tk.DoubleVar(value=0.393)
        self.interval_ms_var = tk.IntVar(value=500)
        self.empty_level_var = tk.DoubleVar(value=0.05)
        self.low_alarm_var = tk.DoubleVar(value=20.0)
        ...
        self.reader: MagnetLHeLevelReader | None = None
        self.running = False
    ...
    def _build(self) -> None:
        ...
        tk.Label(header, text="Magnet LHe Level", bg=self.BG, fg=self.TEXT, font=("Segoe UI Semibold", 26)).grid(row=0, column=0, sticky="w")
        tk.Label(header, text="AMI 135 level probe", bg=self.BG, fg=self.MUTED, font=("Segoe UI", 11)).grid(row=1, column=0, sticky="w")
    ...
    def _readout_panel(self, parent: tk.Widget) -> tk.Frame:
        ...
        tk.Label(panel, text="Magnet LHe Level", bg=self.PANEL, fg=self.MUTED, font=("Segoe UI", 12, "bold")).grid(row=0, column=0, sticky="w", padx=18, pady=(16, 0))
    ...
    def _connection_panel(self, parent: tk.Widget) -> tk.Frame:
        ...
        self._combo(panel, "Resource", self.port_var, ["ASRL5::INSTR", "COM5", "Port6-AMI-135"], 1)
    ...
    def _settings_panel(self, parent: tk.Widget) -> tk.Frame:
        ...
        self._field(panel, "Command", self.command_var, 1)
        self._field(panel, "Scale", self.scale_var, 2)
        self._field(panel, "Poll ms", self.interval_ms_var, 3)
        self._field(panel, "Empty below in", self.empty_level_var, 4)
        self._field(panel, "Low alarm in", self.low_alarm_var, 5)
        self._field(panel, "Overfill in", self.overfill_var, 6)
        tk.Button(panel, text="CSV log...", command=self._choose_csv, bg=self.PANEL_2, fg=self.TEXT, relief="flat").grid(row=7, column=0, columnspan=2, sticky="ew", padx=14, pady=(10, 14))
        panel.columnconfigure(1, weight=1)
        return panel

    def _raw_panel(self, parent: tk.Widget) -> tk.Frame:
        panel = self._panel(parent, "Last Response")
        tk.Label(panel, text="Parsed raw value", bg=self.PANEL, fg=self.MUTED, font=("Segoe UI", 9, "bold")).grid(row=1, column=0, sticky="w", padx=14, pady=(0, 2))
        tk.Label(panel, textvariable=self.raw_value_var, bg="#0b111a", fg=self.CYAN, font=("Consolas", 11, "bold"), anchor="w").grid(row=2, column=0, sticky="ew", padx=14, pady=(0, 8))
        tk.Message(panel, textvariable=self.raw_var, bg="#0b111a", fg=self.TEXT, width=340, font=("Consolas", 10), relief="flat").grid(row=3, column=0, sticky="ew", padx=14, pady=(0, 14))
        panel.columnconfigure(0, weight=1)
        return panel
    ...
    def start(self) -> None:
        ...
        self.running = True
        self.status_var.set("Polling Magnet LHe level...")
        self._poll_once()
    ...
    def _new_reader(self) -> MagnetLHeLevelReader:
        if self.reader is not None:
            self.reader.close()
        command = self.command_var.get().encode("utf-8").decode("unicode_escape")
        return MagnetLHeLevelReader(
            port=self.port_var.get().strip() or "ASRL5::INSTR",
    ...
    def _draw_plot(self) -> None:
        ...
        low_alarm = float(self.low_alarm_var.get())
        overfill = float(self.overfill_var.get())
        lo = min(min(values), low_alarm) - 2
        hi = max(max(values), overfill) + 2
        pts: list[float] = []
    ...
    def _default_csv_path(self) -> Path:
        ...
        return logs / f"magnet_lhe_level_{datetime.now().strftime('%Y%m%d')}.csv"

    def _choose_csv(self) -> None:
        path = filedialog.asksaveasfilename(
            title="Choose Magnet LHe level CSV log",
    ...
    def _write_csv(self, result: MagnetLevelRead) -> None:
        try:
            new_file = not self.csv_path.exists()
            with open(self.csv_path, "a", newline="", encoding="utf-8") as fh:
                writer = csv.writer(fh)
                if new_file:
                    writer.writerow(["date", "time", "timestamp", "level_in", "raw_value", "status", "raw", "port"])
                now = datetime.now()
                writer.writerow([
                    now.strftime("%Y-%m-%d"),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Magnet LHe level GUI")
    parser.add_argument("--port", default="ASRL5::INSTR")
